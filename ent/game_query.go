...
// QueryGameParticipants chains the current query on the "game_participants" edge.
func (gq *GameQuery) QueryGameParticipants() *UserQuery {
	query := &UserQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(game.Table, game.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, game.GameParticipantsTable, game.GameParticipantsPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(gq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (gq *GameQuery) QueryParticipants() *GameParticipantQuery {
	query := &GameParticipantQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(game.Table, game.FieldID, selector),
			sqlgraph.To(gameparticipant.Table, gameparticipant.GameColumn),
			sqlgraph.Edge(sqlgraph.O2M, true, game.ParticipantsTable, game.ParticipantsColumn),
		)
		fromU = sqlgraph.SetNeighbors(gq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (gq *GameQuery) Clone() *GameQuery {
	...
	return &GameQuery{
		...
		withGameParticipants: gq.withGameParticipants.Clone(),
...
// WithGameParticipants tells the query-builder to eager-load the nodes that are connected to
// the "game_participants" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GameQuery) WithGameParticipants(opts ...func(*UserQuery)) *GameQuery {
	query := &UserQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withGameParticipants = query
	return gq
}
...
func (gq *GameQuery) WithParticipants(opts ...func(*GameParticipantQuery)) *GameQuery {
	query := &GameParticipantQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GameQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Game, error) {
	...
	if query := gq.withCreator; query != nil {
		if err := gq.loadCreator(ctx, query, nodes, nil,
			func(n *Game, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	if query := gq.withGameParticipants; query != nil {
		if err := gq.loadGameParticipants(ctx, query, nodes,
			func(n *Game) { n.Edges.GameParticipants = []*User{} },
			func(n *Game, e *User) { n.Edges.GameParticipants = append(n.Edges.GameParticipants, e) }); err != nil {
			return nil, err
		}
	}
	if query := gq.withParticipants; query != nil {
		if err := gq.loadParticipants(ctx, query, nodes,
			func(n *Game) { n.Edges.Participants = []*GameParticipant{} },
			func(n *Game, e *GameParticipant) { n.Edges.Participants = append(n.Edges.Participants, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range gq.withNamedGameParticipants {
		if err := gq.loadGameParticipants(ctx, query, nodes,
			func(n *Game) { n.appendNamedGameParticipants(name) },
			func(n *Game, e *User) { n.appendNamedGameParticipants(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range gq.withNamedParticipants {
		if err := gq.loadParticipants(ctx, query, nodes,
			func(n *Game) { n.appendNamedParticipants(name) },
			func(n *Game, e *GameParticipant) { n.appendNamedParticipants(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (gq *GameQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Game, init func(*Game), assign func(*Game, *User)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_created_games" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (gq *GameQuery) loadGameParticipants(ctx context.Context, query *UserQuery, nodes []*Game, init func(*Game), assign func(*Game, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(game.GameParticipantsTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(game.GameParticipantsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(game.GameParticipantsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(game.GameParticipantsPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "game_participants" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (gq *GameQuery) loadParticipants(ctx context.Context, query *GameParticipantQuery, nodes []*Game, init func(*Game), assign func(*Game, *GameParticipant)) error {
	...
	query.Where(predicate.GameParticipant(func(s *sql.Selector) {
		s.Where(sql.InValues(game.ParticipantsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "game_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedGameParticipants tells the query-builder to eager-load the nodes that are connected to the "game_participants"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (gq *GameQuery) WithNamedGameParticipants(name string, opts ...func(*UserQuery)) *GameQuery {
	query := &UserQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	if gq.withNamedGameParticipants == nil {
		gq.withNamedGameParticipants = make(map[string]*UserQuery)
	}
	gq.withNamedGameParticipants[name] = query
	return gq
}

// WithNamedParticipants tells the query-builder to eager-load the nodes that are connected to the "participants"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (gq *GameQuery) WithNamedParticipants(name string, opts ...func(*GameParticipantQuery)) *GameQuery {
	query := &GameParticipantQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	if gq.withNamedParticipants == nil {
		gq.withNamedParticipants = make(map[string]*GameParticipantQuery)
	}
	gq.withNamedParticipants[name] = query
	return gq
}

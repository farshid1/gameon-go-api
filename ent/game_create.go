...
func (gc *GameCreate) AddParticipantIDs(ids ...int) *GameCreate {
	gc.mutation.AddParticipantIDs(ids...)
	return gc
}

// AddParticipants adds the "participants" edges to the User entity.
func (gc *GameCreate) AddParticipants(u ...*User) *GameCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return gc.AddParticipantIDs(ids...)
}
...
func (gc *GameCreate) createSpec() (*Game, *sqlgraph.CreateSpec) {
	...
	if nodes := gc.mutation.ParticipantsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
